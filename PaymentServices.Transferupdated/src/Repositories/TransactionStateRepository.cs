...
public interface ITransactionStateRepository
{
    ...
    /// <summary>
    /// Reads the current transaction state for an evolveId, or null if the
    /// document doesn't exist. Used for idempotency on Service Bus redelivery.
    /// </summary>
    Task<TransactionState?> GetStateAsync(
        string evolveId,
        CancellationToken cancellationToken = default);
}
...
public sealed class TransactionStateRepository : ITransactionStateRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError("Transaction not found. EvolveId={EvolveId}", evolveId);
            throw;
        }
    }

    public async Task<TransactionState?> GetStateAsync(
        string evolveId,
        CancellationToken cancellationToken = default)
    {
        try
